...
def save_widget_as_image(widget, parent=None, infoText=None, default_path=""):
    from PyQt5.QtWidgets import QFileDialog, QMessageBox
    file_path, _ = QFileDialog.getSaveFileName(
        parent, "스냅샷 저장", default_path, "PNG Files (*.png)"
    )
    if not file_path:
        return
    pixmap = widget.grab()
    if not pixmap.save(file_path):
        QMessageBox.critical(parent, "오류", "이미지 저장 실패")
    else:
        if infoText is not None:
            infoText.append(f"스냅샷 이미지 저장 완료: {file_path}") 

def save_widget_as_image_auto(widget, save_path, infoText=None):
    pixmap = widget.grab()
    if not pixmap.save(save_path):
        from PyQt5.QtWidgets import QMessageBox
        QMessageBox.critical(None, "오류", f"이미지 저장 실패: {save_path}")
    else:
        if infoText is not None:
            infoText.append(f"스냅샷 이미지 저장 완료: {save_path}")

def save_snapshot_auto(widget, infoText=None):
    # 프로젝트 루트 기준으로 snapshots 폴더 생성
